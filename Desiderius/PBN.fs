...
module PBN = 
  let getDealer (s : string) = 
    ...
    | _ -> failwith <| sprintf "unrecognized direction: %c" seventElement
  
  let getElem i list = List.nth list i
  ...
  let getHandsString (s : string) = 
    ...
    let secondPart = 
      afterColonString.Split '\"'
      |> Array.toList
      |> List.head
    secondPart.ToString()
  
  let makeCardFromSuitandRank (s : Desi.Suit) (r : Desi.Rank) = Desi.Card(s, r)
  ...
  let rankFromChar s = 
    ...
    | '3' -> Desi.Rank.Three
    | '2' -> Desi.Rank.Two
    | _ -> failwith <| sprintf "Char '%c' cannot be read as rank" s
  
  let toHandfromOneSuit (suit : Desi.Suit, s : string) : Desi.Card List = 
    s.ToCharArray()
    |> Array.toList
    |> List.map rankFromChar
    |> List.map (makeCardFromSuitandRank suit)
  
  let flatten list = List.concat list
